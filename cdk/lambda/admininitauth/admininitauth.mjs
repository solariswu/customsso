...
const headers = {
  "Access-Control-Allow-Headers":
    "Content-Type,Authorization,X-Api-Key,Set-Cookie,Cookie,X-Requested-With",
...
const validateInputParams = (event) => {
  return (
    ...
    (event.password.trim().length > 0 || event.passkey)
  );
};
...
const genSessionID = async (username, apti, session, dynamodb) => {
  console.log("generating session id for user:", username, " apti:", apti, " session", session);
...
export const handler = async (event) => {
  ...
  let error = { message: "admin_initauth error" };

  const payload = JSON.parse(event.body);
  console.log("admin_initauth payload:", payload);
  ...
  if (validateInputParams(payload)) {
    try {
      ...
      if (user.ChallengeName === "WEB_AUTHN") {
        const uuid = await genSessionID(payload.email, payload.apti, user.Session, dynamodb);
        const res = { email: payload.email, apti: payload.apti, uuid, webauthn: user.ChallengeParameters };
        return {
          statusCode: 204,
          headers,
          body: JSON.stringify(res),
