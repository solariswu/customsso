...
const salt = 'some_long_key'; // Pull this from a property file. All MFA services will use this same salt to read and write the one_time_token-Cookie.
const asmurl = 'https://asm2.apersona.com:8443/asm';  // Url of the Adaptive MFA Server.


let l = 'epnd-su-72ja37bc51mz';  
...
let wr = 'epnd.com'; // This is the domain of the services where MFA is being setup. It must match the domain for the HTTPS URL domain where the cookie is secured.
...
let u = 'ksparksnc@icloud.com'; // email address of the user. Entered from front end web service to login via post.
let c = '278dcbdee5660876c230650ebb4bd70e';  // For every new MFA Auth login the nodeJS backend needs to read the cookie from teh client if it exists and send it in.
let igd = 0;  // On the main login page, add a checkbox:  [ ] Remember this device, I own it. If checked, set igd = 0 and send it with all related transactions until the login process completes 
...
let a = encodeURI('{"apersonaKey":"36b284dffbd5-956bf51bb14ce-cfd6ce53e8c87d4","deviceType":"PC","osInfo":"Windows - 7","deviceInfo":"device_info_here","browserInfo":"Chrome - 32.0.1700.107","webHost":"10.5.1.5:8080","pageUrl":"/svcs/login.ap"}');
...
let uIp = '45.23.45.12'; // This is the client ip address as detected from the NodeJS server. see: https://www.abstractapi.com/guides/node-js-get-ip-address Let's use request.header.
let apti = '123456'; // This key needs to be a randon key that is set for each new login process and kept and sent in until the user succeeds or fails their login.
...
axios.post(
	asmurl + '/extAuthenticate.kv?' + 'l=' + l + '&u=' + u + '&uIp=' + uIp + '&apti=' + apti + '&c=' + c + '&wr=' + wr + '&igd=' + igd + '&otpm=' + otpm + '&p=' + p + '&otpp=' + otpp + '&sfl=' + sfl + '&nsf=' + nsf + '&tType=' + tType + '&af1=' + af1 + '&a=' + a)
	.then(function (res) {
