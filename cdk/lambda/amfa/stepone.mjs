...
const configs = await axios.get('https://cdn.jsdelivr.net/gh/solariswu/free-cdn-source@main/amfa-test-config.json');

console.log(configs);
// API vars saved in node.js property file in the back-end node.js
const salt = configs.salt; // Pull this from a property file. All MFA services will use this same salt to read and write the one_time_token-Cookie.
...
export const stepone = async (event) => {
	...
	const response = await client.send(new AdminListGroupsForUserCommand(param));

	console.log ('get group res:', response);
