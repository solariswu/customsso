...
export const stepone = async (event) => {
    const getTenantDataResponse = await axios
    .get(process.env.TENANT_CONFIG_URL)
    .catch((err) => {
      console.log('Error getting tenant data:', err);
      return response(500, 'Error getting tenant data');
    });

  if (getTenantDataResponse.status !== 200) {
    console.log('err-getTenantDataResponse', getTenantDataResponse);
    return response(500, 'Error getting tenant data');
  }

  const tenantData = getTenantDataResponse.data;



  const getConfigResponse = await axios
    .get(process.env.AMFA_CONFIG_URL)
    .catch((err) => {
      console.log('Error getting config file:', err);
      return response(500, 'Error getting amfa config file');
    });

  if (getConfigResponse.status !== 200) {
    console.log('Error getting config file:', getConfigResponse);
    return response(500, 'Error getting amfa config file');
  }

  const amfaConfigs = getConfigResponse.data;
  ...
  const userGroup = await client.send(new AdminListGroupsForUserCommand(param)).catch(err => {
    console.log('Error getting group list for user:', err);
    return { statusCode: 500, body: 'Error getting group list for user' };
  });
  ...
  console.log('stepone sending data:', asmurl +
    '/extAuthenticate.kv?' +
    'l=' +
    l +
    '&u=' +
  ...
  console.log('stepone response:', response);
}
