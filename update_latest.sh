...
if ! [ -f ./config.sh ]; then
  ...
  mv config.sh .amfa_config_bk && rm -rf $APERSONAIDP_REPO_NAME/ $APERSONAADM_REPO_NAME/ *.sh *.json *.txt && curl -o- $INIT_DEPLOY_SCRIPT |bash && cp .amfa_config_bk config.sh && ./install.sh
fi
